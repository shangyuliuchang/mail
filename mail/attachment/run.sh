...
courses=($($py_command ./canvas.py xxx -f -d $path | tr -d '[],'))
rm /tmp/temp.txt
# echo ${#courses[@]}
len_course=${#courses[@]}
# echo ${courses[@]}
...
do
	...
	$py_command ./canvas.py $course_name -d $path
...
do
	course_name=${courses[$(($i + 1))]}
	delta=`$py_command ./canvas.py $course_name -m ${courses[0]} -d $path`
	my_activity=$((my_activity + $delta))
	$py_command ./canvas.py $course_name -r -d $path
done
$py_command ./canvas.py xxx -s $my_activity -d $path
$py_command ./canvas.py xxx -p -d $path