...
courses=($(python3 ./canvas.py xxx -f -d $path | tr -d '[],'))
rm /tmp/temp.txt
# echo ${#courses[@]}
len_course=${#courses[@]}

python3 ./canvas.py xxx -c -d $path
...
do
	...
	python3 ./canvas.py $course_name -d $path
...
do
	course_name=${courses[$(($i + 1))]}
	delta=`python3 ./canvas.py $course_name -m ${courses[0]} -d $path`
	my_activity=$((my_activity + $delta))
	python3 ./canvas.py $course_name -r -d $path
done
python3 ./canvas.py xxx -s $my_activity -d $path
python3 ./canvas.py xxx -p -d $path